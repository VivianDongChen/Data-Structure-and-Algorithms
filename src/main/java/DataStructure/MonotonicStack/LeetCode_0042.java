 ...
 * 单调栈的定义
 * - 单调递增栈：栈中的元素从栈底到栈顶按递增顺序排列（即每个元素都小于等于其后来的元素）。
 *   - 每次入栈时，若新元素小于等于栈顶元素，则直接入栈；
 *   - 若新元素大于栈顶元素，则弹出栈顶元素，直到栈顶元素不再小于新元素或栈为空。
 * - 单调递减栈：栈中的元素从栈底到栈顶按递减顺序排列（即每个元素都大于等于其后来的元素）。
 *   - 每次入栈时，若新元素大于等于栈顶元素，则直接入栈；
 *   - 若新元素小于栈顶元素，则弹出栈顶元素，直到栈顶元素不再大于新元素或栈为空。
 */
